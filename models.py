...
from util import create_instructions
...
class CQD(nn.Module):
	...
	def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
		...
		for query_structure, queries in batch_queries_dict.items():
			...
			self.graph_type = query_name_dict[query_structure]
			...
			if 'co' in self.method:
				...
				for atom in range(len(atoms[0])):
					part = atoms[:,atom,:]
					chain = self.get_full_embeddigns(part)
					chains.append(chain)

				print(chains[0][0].shape)
				scores_bf = self.query_answering_BF(chains, chain_instructions, self.graph_type,  5, t_norm = 'prod', batch_size = 1, scores_normalize = 'default')


			scores = torch.cat(all_scores, dim=0) if len(all_scores) > 0 else scores_bf
	...
	def query_answering_BF(self,chains, chain_instructions, graph_type, candidates: int = 5, t_norm: str = 'min', batch_size = 1, scores_normalize = 0):
