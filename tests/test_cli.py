...
@pytest.mark.light
def test_cli_continuous_v1():
    env = os.environ.copy()
    env['PYTHONPATH'] = '.'

    cmd_str = 'python3 main.py --do_train --do_valid --do_test --data_path data/NELL-betae -n 1 -b 10 -d 100 ' \
              '-lr 0.1 --disable_warmup --max_steps 10 --cpu_num 0 --geo cqd --cqd co --valid_steps 10 --tasks 1p ' \
...
def test_cli_d2_v1():
    ...
    cmd_str = 'python3 main.py --do_train --do_valid --do_test --data_path data/NELL-betae -n 1 -b 10 -d 100 ' \
              '-lr 0.1 --disable_warmup --max_steps 10 --cpu_num 0 --geo cqd --cqd d2 --valid_steps 10 --tasks 1p ' \
              '--print_on_screen --test_batch_size 1000 --optimizer Adagrad --reg_weight 0.01 --log_steps 500 ' \
              '--no-save'
